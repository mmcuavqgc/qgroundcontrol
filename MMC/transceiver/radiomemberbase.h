...
//    MMC_GENERATE_QML_PROPERTY(rockerState, uint, "rockerState")
    ...
    MMC_GENERATE_QML_PROPERTY(voltage, float, "voltage")
    /* SBUS Count */
    MMC_GENERATE_QML_PROPERTY(sbusCount, int, "sbusCount")
    /* 喊话器发送数据计数*/
    MMC_GENERATE_QML_PROPERTY(micCount, int, "micCount")
    /* PC发送数据计数*/
    MMC_GENERATE_QML_PROPERTY(pcCount, int, "pcCount")
    /* 飞控发送数据计数*/
    MMC_GENERATE_QML_PROPERTY(vehicleCount, int, "vehicleCount")
public:
    ...
    Q_PROPERTY(QString      radioID             READ radioID            NOTIFY radioIDChanged)
    /* 当前电台开关状态 0-关，1-开*/
    Q_PROPERTY(int          rockerState         READ rockerState        WRITE  setRockerState       NOTIFY rockerStateChanged)
    /* 当前遥控器选择的类型 */
    Q_PROPERTY(int          teleControlType     READ teleControlType    WRITE  setTeleControlType   NOTIFY teleControlTypeChanged)

public:
    virtual QString radioSettingUrl() = 0;
    virtual void analysisPack(int type, QByteArray msg) = 0;

//    int     sbusCount() { return _sbusCount;}
    int   radioLock  (){return _radioLock; }
    void  setRadioLock(bool lock);
    QString radioID  (){return _radioID; }
    void setRadioID(QString id);
    ...
    int teleControlType()   { return _teleControlType; }

    int rockerState()       { return _rockerState;}
//    void setSbusCount(int count);
    void setTeleControlType(int type);   //切换遥控器 1为神机军师II控制，2位futaba控制
    void setRockerState(int state);   /* 当前电台开关状态 0-关，1-开*/
    /* 下发操作 */
    void queryRadioId();          //查询单片机唯一ID
    /* 设置遥控器模式 */
    Q_INVOKABLE void setCalirationState(bool isLeft);   //true-左手模式 false-右手模式
    Q_INVOKABLE void sendCheckStatus();                 //摇杆校准
    Q_INVOKABLE void rockerControl(int state);          //电台开关，0-关闭 1-开启

signals:
    void _writeData(char, QByteArray);
    void radioLockChanged();
    void radioIDChanged();
    void teleControlTypeChanged();
    void rockerStateChanged(int state);
//    void sbusCountChanged();
